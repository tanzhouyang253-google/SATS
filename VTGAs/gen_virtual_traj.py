...
from mcts import generate_virtual_trajectory
...
def speed_obf(nav, seq):
    ...
    while q < len_nav:
        ...
        dis = geodesic((nav[q][1], nav[q][0]), (new_nav[new_n][1], new_nav[new_n][0])).meters
        spe = dis / 5
        
        if abs(spe - tar) < 1.1: # 速度差值的阈值 1.1？
...
def get_false_from_true(real_traj):
    ...
    # if speed_ave < 3:
    #     nav_traj = generate_virtual_trajectory(st, en, mode = 'walk')
    # elif 10 >= speed_ave >= 3:
    #     nav_traj = generate_virtual_trajectory(st, en, mode = 'bike')
    # else:
    #     nav_traj = generate_virtual_trajectory(st, en, mode = 'drive')
    ...
    # 8. 添加时间戳， 5s 一个点；false_traj 为最后生成的虚假轨迹
    false_traj = []
    for i, val in enumerate(tmp):
        false_traj.append([val[0], val[1], i * 5])
...
def get_a_trajectory(path):
    ...
    for line in fp:
        ...
        lng = round(float(line_list[2]), 6) # 经度
        lat = round(float(line_list[3]), 6) # 纬度
        time_str = line_list[1]  # 时间字符串
        
        dt = datetime.strptime(time_str, "%Y-%m-%d %H:%M:%S")
        tim = round(dt.timestamp(), 1)  # 转换为 Unix 时间戳
        
        pts.append([lng, lat, tim])
        pts_no.append([lng, lat])
    fp.close()
    return pts
...
if __name__ == '__main__':
    ...
    path = "F:\\Desktop_File\\copy\\AnotherMe-master\\real"
    ...
    for i, f in enumerate(files):
        ...
        # for val in b:
        #     val[2] += a[0][2]
        p = "F:\\Desktop_File\\copy\\AnotherMe-master\\false\\" + file_list[i] # store path
        print(p)
        store_path(b, p)
